...
def animate(mode, slice_val, slabs, x, y, z, filename, title_str="", fps=24, units_label=""):
    if mode == "xz":
        ...
        figsize, xl, yl = (8, 4), "x (m)", "depth below surface (m)\n(AZMP 3-95 m compressed to 40 m)"
    else:
        H, V = np.meshgrid(x, y)
    ...
    fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap="viridis"),
                 cax=cax, label=units_label)
    fig.get_layout_engine().set(rect=(0, 0, 0.88, 1))
    ax.set_xlim(x[0], x[-1])
    if mode == "xz":
        ax.set_ylim(V.max(), V.min())   # depth 0 (surface) top, 40 (seabed) bottom
    else:
    ...
    if mode == "xz" and abs(slice_val - CYL_Y) < CYL_R:
        ...
        ax.set_aspect("equal", adjustable="box")  # cylinder renders as a true circle
    ax.autoscale(False)
