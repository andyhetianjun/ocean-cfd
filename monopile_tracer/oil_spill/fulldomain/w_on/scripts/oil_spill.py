...
# --- oil spill config -------------------------------------------------
OIL_MASS_KG   = 573.0       # 7.96 g/m2 over 72000 m2, matches disc runs
OIL_RADIUS    = 20.0        # m, D = 40 m
OIL_OFFSET    = 26.0        # m, tangent 24 + 2 m
BEARING       = "N"      # unused when FULL_DOMAIN
OIL_FULL_DOMAIN = True   # oil covers the entire 600 x 120 surface
VERTICAL_MIX  = True
...
def build_oil_field(x, y, z_tr):
    ...
    if OIL_FULL_DOMAIN:
        ...
        disc_area = (float(x[-1]-x[0]) + dx) * (float(y[-1]-y[0]) + dy)
    else:
...
def evolve(C0, x, y, z_vel, z_tr, jobs):
    C = C0.copy()
    C_INLET = C0.copy() if OIL_FULL_DOMAIN else None
    ...
    for n, t in enumerate(TIMESTEPS):
        ...
        C[outside] = C_INLET[outside] if OIL_FULL_DOMAIN else 0.0
...
def main():
    ...
    nut = "Oil"; fp = "oil_fulldomain_case2"
    ...
    jobs = [("xy", float(z_tr[-1]), f"{fp}_xy_z0_surface.gif",
             f"{nut}  |  X-Y plane at z = 0 m (surface), full-domain initial slick, with no buoyancy")]
    import time
