...
OUT_DIR = "velocity_full_025"
BOUNDS = ((0.0, 600.0), (0.0, 120.0), (-20.0, 20.0))
SHAPE = (2400, 480, 84)
WORKERS = 24
N_PROCS = 256
MAX_TIMESTEPS = 400
...
def main():
    ...
    todo = [t for t in times if (1049 <= float(t) <= 1400) and not os.path.exists(os.path.join(OUT_DIR, f"vel_{t}.npz"))]
    ...
    with Pool(WORKERS) as pool:
        for n, t in enumerate(todo, 1):
            ...
            _c3 = counts.reshape(SHAPE[2], SHAPE[1], SHAPE[0])
            _empty = _c3 == 0
            if _empty.any():
                from scipy import ndimage
                _idx = tuple(ndimage.distance_transform_edt(
                    _empty, return_distances=False, return_indices=True))
                for _c in range(3):
                    gvel[_c] = gvel[_c][_idx]
                print(f"      filled {100.0*_empty.mean():.1f}% empty bins", flush=True)
